...
public class CommentController {
    ...
    @PostMapping("/comment/{postId}") // @RequestHeader(name = "Authorization") 상수로 정해주는게 유지보수 측면에서 아쉬움(Header name이 바뀌면 코드를 다 바꿔줘야함) // jwtUtil.AUTHORIZATION_HEADER로 받아오면 가능 // 근데 Controller에 JwtUtil을 주입받는게 별로임 // 찾아보니 import static 으로 상수를 가져올 수 있다. 대신 가독성이 떨어지는듯
    ...
    public ResponseEntity<String> deleteComment(@RequestHeader(name = AUTHORIZATION_HEADER) String token,
                                                @PathVariable Long id,
                                                @PathVariable Long postId) {
        return commentService.deleteComment(token, id, postId);
    }
}
